...
void pars(string namefrom,string nameto, string message, boost::shared_array<char> arr)
{
	arr[0]=1;
	arr[1]=0;
...
public:
	...
	void vectempty(size_t ind)
	{
		...
		v.erase(v.begin(),v.begin()+i);
		cout << "vector [0]" << static_cast<int>(v[0]) << static_cast<int>(v[1]) << endl;
	}
			...
			if(a[10+Getlen(a,5)+Getlen(a,8+Getlen(a,5))]==2)
			{
				...
				if(a[10+Getlen(a,5)+Getlen(a,8+Getlen(a,5))]==1)
				{
					...
					if(a[10+Getlen(a,5)+Getlen(a,8+Getlen(a,5))]==0)
					{
						...
						vectempty(10+Getlen(a,5)+Getlen(a,8+Getlen(a,5)));
						throw err; //
					}
...
void findresult(boost::shared_array<char> a,Msg &msg)
{
	if(a[0]==1 && a[1]==0)
	{
